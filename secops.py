    ...
    def _discover_tools(self):
        ...
        tool_definitions = {
            "eml": {
                ...
                "examples": ["secops eml suspicious.eml --vt", "secops eml phishing.eml --output report.json"],
            ...
            "ioc": {
                ...
                "examples": ["secops ioc threat_report.txt", "secops ioc --file report.txt --format csv --defang"],
                "requires_api": [],
            },
            "hash": {
                "name": "Hash Lookup",
                ...
                "examples": ["secops hash 44d88612fea8a8f36de82e1278abb02f", "secops hash --file hashes.txt --verbose"],
                "requires_api": ["VT_API_KEY (optional)"],
            },
            "intel": {
            ...
            "log": {
                ...
                "examples": ["secops log /var/log/apache2/access.log", "secops log nginx.log --type nginx --format txt"],
                "requires_api": [],
            },
            "pcap": {
                "name": "PCAP Analyzer",
                ...
                "examples": ["secops pcap capture.pcap", "secops pcap traffic.pcapng --verbose --output analysis.json"],
                "requires_api": [],
            },
            "url": {
                "name": "URL Analyzer",
                ...
                "examples": ['secops url "http://suspicious-site.com"', "secops url --file urls.txt --format json"],
                "requires_api": ["VT_API_KEY (optional)"],
            },
            "yara": {
                "name": "YARA Scanner",
            ...
            "cert": {
                ...
                "examples": ["secops cert https://example.com", "secops cert --file cert.pem --hostname example.com"],
                "requires_api": [],
            },
            "deobfuscate": {
...
class ToolManager:
    ...
    def run_tool(self, tool_id: str, args: List[str]):
        ...
        if not tool["available"]:
            print(f"Error: Tool '{tool_id}' not found at {tool.get('path', 'unknown path')}", file=sys.stderr)
            sys.exit(1)
            ...
            if quiet:
                ...
                print(
                    reporter.format_json(
                        result["input"], result["type"], result["scorer"], result["iocs"], result["tool_results"]
                    )
                )
            elif verbose:
                print(
