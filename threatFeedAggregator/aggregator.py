...
class ThreatFeedStorage:
    ...
    def store_ioc(self, ioc: Dict) -> bool:
        ...
        try:
            ...
            if existing:
                ...
                new_source = {"name": ioc.get("source", "unknown"), "first_seen": ioc.get("first_seen", now)}
                ...
                cursor.execute(
                    ...
                    WHERE ioc_hash = ?
                """,
                    (now, new_confidence, json.dumps(existing_sources), json.dumps(ioc.get("tags", [])), ioc_hash),
                )
                ...
                """,
                    (
                        ...
                        json.dumps([{"name": ioc.get("source", "unknown"), "first_seen": ioc.get("first_seen", now)}]),
                        now,
                    ),
                )
    ...
    def search_ioc(
        self, value: str = None, ioc_type: str = None, malware_family: str = None, min_confidence: int = 0, limit: int = 100
    ) -> List[Dict]:
        """Search for IOCs"""
        cursor = self.conn.cursor()
    ...
    def record_update(
        self, source_name: str, iocs_added: int, iocs_updated: int, success: bool = True, error_message: str = None
    ):
...
class ThreatFoxFeed:
    ...
    def fetch_recent(self, days: int = 1) -> List[Dict]:
        ...
        try:
            ...
            response = requests.post(self.base_url, json={"query": "get_iocs", "days": days}, timeout=30)
...
class URLhausFeed:
    ...
    def fetch_recent(self, limit: int = 100) -> List[Dict]:
        ...
        try:
            ...
            response = requests.post(self.base_url + "urls/recent/", json={"limit": limit}, timeout=30)
...
class FeedAggregator:
    ...
    def __init__(self, storage: ThreatFeedStorage, verbose=False):
        ...
        self.verbose = verbose
        self.feeds = {"threatfox": ThreatFoxFeed(verbose=verbose), "urlhaus": URLhausFeed(verbose=verbose)}
...
def main():
    ...
    # Export command
        ...
        results = storage.search_ioc(min_confidence=args.min_confidence, limit=10000)  # Large limit for export
