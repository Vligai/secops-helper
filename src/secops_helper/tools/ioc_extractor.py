...
class IOCExtractor:
    ...
    def get_summary(self, results: Dict) -> Dict:
        ...
        summary['total_iocs'] += (
            summary['ips'] + summary['domains'] +
            summary['urls'] + summary['emails'] + summary['cves']
        )
...
Examples:
  ...
  python extractor.py report.txt --defang --output safe_report.txt
    ...
    parser.add_argument(
        '--whitelist', '-w',
        help='Path to whitelist file (domains to exclude)'
    )

    parser.add_argument(
        '--no-private-ips',
        action='store_true',
        help='Exclude private IP addresses'
    )

    parser.add_argument(
        '--defang',
        action='store_true',
        help='Defang IOCs in output (make safe for sharing)'
    )

    parser.add_argument(
        '--refang',
        action='store_true',
        help='Refang defanged IOCs (convert to active form)'
    )
