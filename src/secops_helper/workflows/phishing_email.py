...
from secops_helper.core.workflow import Workflow, WorkflowStep, StepResult, WorkflowContext, workflow
...
class PhishingEmailWorkflow(Workflow):
    ...
    def _define_steps(self):
        self.steps = [
            ...
            WorkflowStep(
                ...
                description="Calculate phishing probability score",
                tool="scorer",
                required=True
            )
        ]
    ...
    def _parse_email(self, context: WorkflowContext) -> StepResult:
        ...
        try:
            ...
            if spf.get('result') == 'fail':
                ...
                context.scorer.add_finding(
                    Severity.MEDIUM,
                    "SPF soft fail - sender authentication weak",
                    "eml_parser",
                    {'spf': spf}
                )
            ...
            return StepResult(step_name="parse_email", success=False,
                            error="EML parser not available")
    ...
    def _extract_iocs(self, context: WorkflowContext) -> StepResult:
        ...
        try:
            ...
            return StepResult(step_name="extract_iocs", success=False,
                            error="IOC extractor not available")
        except Exception as e:
    ...
    def _check_hashes(self, context: WorkflowContext) -> StepResult:
        ...
        if not hashes:
            return StepResult(step_name="check_hashes", success=True,
                            data={'message': 'No hashes to check'})
        ...
        try:
            ...
            for hash_val in hashes[:5]:  # Limit to 5 to avoid rate limiting
                ...
                if result.get('verdict') == 'malicious':
                    context.scorer.add_finding(
                        Severity.CRITICAL,
                        f"Attachment hash is MALICIOUS: {hash_val[:16]}...",
                        "hash_lookup",
                        result
                    )
                    ...
                    context.scorer.add_finding(
                        Severity.HIGH,
                        f"Attachment hash is suspicious: {hash_val[:16]}...",
                        "hash_lookup",
                        result
                    )
            ...
            return StepResult(step_name="check_hashes", success=False,
                            error="Hash lookup not available")
        except Exception as e:
    ...
    def _check_domains(self, context: WorkflowContext) -> StepResult:
        ...
        if not domains:
            return StepResult(step_name="check_domains", success=True,
        ...
        try:
            ...
            return StepResult(step_name="check_domains", success=False,
                            error="Domain intel not available")
        except Exception as e:
    ...
    def _check_urls(self, context: WorkflowContext) -> StepResult:
        ...
        if not urls:
            return StepResult(step_name="check_urls", success=True,
                            data={'message': 'No URLs to check'})
        ...
        try:
            ...
            return StepResult(step_name="check_urls", success=False,
                            error="URL analyzer not available")
        except Exception as e:
    ...
    def _check_certificates(self, context: WorkflowContext) -> StepResult:
        ...
        if not urls:
            return StepResult(step_name="check_certificates", success=True,
                            data={'message': 'No HTTPS URLs to check'})
        ...
        try:
            ...
            for url in urls[:3]:  # Limit checks
                try:
                    ...
                    if result.get('verdict') == 'suspicious':
                        context.scorer.add_finding(
                            Severity.HIGH,
                            f"Certificate issues detected for: {url[:50]}",
                            "cert_analyzer",
                            result
                        )
            ...
            return StepResult(step_name="check_certificates", success=False,
                            error="Certificate analyzer not available")
        except Exception as e:
