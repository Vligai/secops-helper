...
class NetworkForensicsWorkflow(Workflow):
    ...
    def _define_steps(self):
        self.steps = [
            WorkflowStep(
                name="parse_pcap",
                description="Parse network capture",
                tool="pcap_analyzer",
                required=True
            ),
            WorkflowStep(
    ...
    def _parse_pcap(self, context: WorkflowContext) -> StepResult:
        ...
        try:
            ...
            return StepResult(step_name="parse_pcap", success=False,
                            error="PCAP analyzer not available")
    ...
    def _extract_iocs(self, context: WorkflowContext) -> StepResult:
        ...
        context.data['network_iocs'] = {
            'unique_ips': len(ips),
            'dns_queries': len(domains)
        }

        return StepResult(step_name="extract_iocs", success=True,
                         data=context.data['network_iocs'])
    ...
    def _detect_scans(self, context: WorkflowContext) -> StepResult:
        ...
        return StepResult(step_name="detect_scans", success=True,
                         data={'port_scans': len(port_scans)})
    ...
    def _analyze_dns(self, context: WorkflowContext) -> StepResult:
        ...
        return StepResult(step_name="analyze_dns", success=True,
    ...
    def _check_ips(self, context: WorkflowContext) -> StepResult:
        ...
        if not public_ips:
            return StepResult(step_name="check_ips", success=True,
        ...
        try:
            ...
            for ip in public_ips:
                ...
                if result.get('verdict') == 'malicious':
                    ...
                    context.scorer.add_finding(
                        Severity.CRITICAL,
                        f"Communication with malicious IP: {ip}",
                        "domain_intel",
                        result
                    )

            return StepResult(step_name="check_ips", success=True,
                            data={'checked': len(public_ips), 'malicious': malicious_count})
        ...
        try:
            ...
            if matches:
                for match in matches:
                    context.scorer.add_finding(
                        Severity.HIGH,
                        f"YARA match in network traffic: {match.get('rule')}",
                        "yara_scanner",
                        match
                    )
    ...
    def _generate_report(self, context: WorkflowContext) -> StepResult:
        ...
        if summary['risk_score'] >= 70:
            ...
            recommendations.extend([
                "Monitor identified IPs for additional activity",
